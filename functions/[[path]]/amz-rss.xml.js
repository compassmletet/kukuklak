...
export async function onRequest(context) {
  ...
  try {
    ...
    // PARSING URL BERDASARKAN STRUKTUR BARU
    const pathSegments = params.path || [];
    const categoryParam = pathSegments[0]; 
    const usernameParam = pathSegments[1]; 
    const domainParam = pathSegments[2]; // Path untuk Domain baru
    const pintUserParam = pathSegments[3];
    const pintBoardParam = pathSegments[4];
    const extraBacklinkSegments = pathSegments.slice(5);

    const emailUser = usernameParam || DEFAULT_EMAIL_USER; 
    
    // Perbaikan Logika Email: Mengambil langsung dari pathSegments[2]
    const DYNAMIC_EMAIL = domainParam ? `${emailUser}@${domainParam}` : `${emailUser}@${url.host}`; 
...
`;
    ...
    if (results && results.length > 0) {
      for (let i = 0; i < results.length; i++) {
        ...
        const ctaPrefix = spinTextStable("{View on Amazon|Check Price on Amazon|See Details on Kindle Store|View On Audible Amazon|Check Official Amazon Price|See Kindle Store Details|View Audible Edition}", seed + "cta");
